...
void filtering(double *psitot,double *potl,double *ksqr,zomplex *an,double *zn,double *el,lng_st ist,par_st par,long tid,long jns,long *idum,fftw_plan_loc planfw,fftw_plan_loc planbw,fftw_complex *fftwpsi)
{
  ...
  if ((eval = (double*)calloc(ist.two,sizeof(double)))==NULL)nerror("eval");
  ...
  normalize_all(psitot,par.dv,ist.two,ist.ngrid);
  
  /*** calculate and print the energy of the filtered states ***/
  energy_all(psi,phi,psitot,potl,ksqr,eval,ist,par,planfw,planbw,fftwpsi,ist.two);
  ...
  pf = fopen(str , "w");
  fwrite (psitot,sizeof(double),ist.two*ist.ngrid,pf);
  fclose(pf);
