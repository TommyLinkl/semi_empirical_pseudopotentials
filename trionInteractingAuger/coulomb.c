...
{
  FILE *pf; zomplex *rho=NULL; double sum;
  long jms, k, i, j, b, a, c, igrid, tid, *list1=NULL, *list2=NULL, thl, th2l;
  ...
  if ((list1 = (long *) calloc(ist.itot, sizeof(long))) == NULL) nerror("list i");
...
#pragma omp parallel for private(i,k,igrid,j,b,sum,tid)
  for (i = 0; i < ist.itot; i++) {
    ...
    for (k = 0; k < ist.numBandEdgeHoles; k++) {
      for (igrid = 0; igrid < ist.ngrid; igrid++) {
      	rho[tid*ist.ngrid+igrid].re = psiai[list1[i]*ist.ngrid+igrid] * psibs[k*ist.ngrid+igrid];
  ...
  if ((list2 = (long *) calloc(ist.atot, sizeof(long))) == NULL) nerror("list a");
...
#pragma omp parallel for private(a,c,igrid,j,b,sum,tid)
  for (a = 0; a < ist.atot; a++) {
    ...
    for (b = 0; b < ist.numBandEdgeElectrons; b++) {
      for (igrid = 0; igrid < ist.ngrid; igrid++) {
      	rho[tid*ist.ngrid+igrid].re = psiai[list2[a]*ist.ngrid+igrid] * psibs[(b+ist.numBandEdgeHoles)*ist.ngrid+igrid];
      	rho[tid*ist.ngrid+igrid].im = 0.0;
      }
      hartree(&rho[tid*ist.ngrid],potq,&pothhl[tid*ist.ngrid],ist,planfw[tid],planbw[tid],&fftwpsi[tid*ist.ngrid]);
