...
void generate_filter_states(double *psitot,double *eval,double *sige,double *evalbe,double *ksqr,double *potl,double *vx,double *vy,double *vz,lng_st *ist,par_st *par,fftw_plan_loc *planfw,fftw_plan_loc *planbw,fftw_complex *fftwpsi)
{
  ...
  writeFunctionStartTime("Program is now generating filtered excited electron and hole states:\n", stdout);

  idum = ist->seed;
  if (idum == 0) {
    Randomize(); idum = -random();
    printf("Random seed used in filtering the excited states: seed = %ld\n", idum); 
  }
  fflush(0);

  // Includes a larger energy window since the energy of the initial biexciton 
  // ranges from a minimum of 2.0*par->Ex (both excitons in lowest energy state) to 2.0*par->Ex+3kbT
  ...
  printf("Min and max excited electron target energies: Eamin = %.6f Eamax = %.6f\n", par->Eamin, par->Eamax);
  ...
  printf("Min and max excited hole target energies: Eimin = %.6f Eimax %.6f\n", par->Eimin, par->Eimax);
