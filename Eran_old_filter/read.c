...
void read_conf(double *rx,double *ry,double *rz,atm_st *atm,long ntot,FILE *pf)
{
  ...
  for (xd = yd = zd = 0.0, i = 0; i < ntot; i++){
    /*fscanf(pf,"%s %lg %lg %lg",&c[0],&rx[i],&ry[i],&rz[i]);*/
    /*fscanf(pf,"%lg %lg %lg",&rx[i],&ry[i],&rz[i]);*/
...
void read_pot(double *vr,double *pot,long *npot,double *dr,atm_st *atm,long n,long ntype)
{
  ...
  a[10] = 0.04;      /*** ligand P3 for Cd ***/
  a[11] = -0.384;    /*** ligand P4 for Se ***/
  b[8] =  2.2287033; /*** ligand P1 for Cd ***/
  b[9] =  2.2287033; /*** ligand P2 for Se ***/
  b[10] = 2.2287033; /*** ligand P3 for Cd ***/
  b[11] = 2.2287033; /*** ligand P4 for Se ***/
  ...
  */
  ...
  for (j = 0; j < ntype; j++) {
    ...
    printf ("length of atom type %ld file %ld\n", j, npot[j]);
    ...
    /*** scaling the Se1 potential ***/
    //for (j = 15, i = 0; i < npot[j]; i++) pot[j*n+i] *= 1.1;
