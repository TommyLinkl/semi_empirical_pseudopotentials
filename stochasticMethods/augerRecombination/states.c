...
long calcNumNonIntBiexcStates(double *evalbe, double maxBiexcEnergy, long nHoles, long nElecs, long nTrappedStates) {
    ...
    for (b = 0; b < nElecs; b++) {
        // exciton with hole at band edge
	for (j = 0; j < (nHoles-nTrappedStates); j++) {
	    for (c = 0; c < nElecs; c++) {
                // if trap states
                if (nTrappedStates > 0) {
                    k = nHoles-1;
...
long fillNonIntBiexcStruct(nonIntBiexc *biExciton, double *eval, double maxEnergy, lng_st ist) {
    ...
    for (b = 0; b < ist.nElecs; b++) { 
        // first initial exciton has hole at band edge
	for (j = 0; j < (ist.nHoles-ist.nTrappedStates); j++) {
	    for (c = 0; c < ist.nElecs; c++) {
                // if trap states, second initial exciton has hole at trap
                if (ist.nTrappedStates > 0) {
                    k = ist.homoIndex; // trapped hole is homo
