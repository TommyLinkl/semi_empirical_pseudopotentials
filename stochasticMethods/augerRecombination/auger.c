                                ...
                                lng_st ist, par_st par, fftw_plan_loc *planfw, fftw_plan_loc *planbw, fftw_complex *fftwpsi)
{
  ...
  for (iDeltaE = 0; iDeltaE < ist.nConsWindows; iDeltaE++) {
    ...
    calcBoltzmannWeightedRates(biexcitonEnergies, &(eRate[iDeltaE*ist.nBiexcitons]), 
            &(hRate[iDeltaE*ist.nBiexcitons]), par.temp, ist.nBiexcitons, pf, iDeltaE); 
    writeSeparation(pf);
  }
  fclose(pf);
                                    ...
                                    fftw_plan_loc *planfw, fftw_plan_loc *planbw, fftw_complex *fftwpsi)
{
  ...
  for (iDeltaE = 0; iDeltaE < ist.nConsWindows; iDeltaE++) {
    fprintf(pf, "Energy conservation window = %.6f\n\n", deltaE[iDeltaE]);
    calcBoltzmannWeightedRates(biexcitonEnergies, &(eRate[iDeltaE*ist.nBiexcitons]), 
            &(hRate[iDeltaE*ist.nBiexcitons]), par.temp, ist.nBiexcitons, pf, iDeltaE); // TODO: remove -1 from ist.nBiexcitons
    writeSeparation(pf);
  }
  fclose(pf);
                     ...
                     double *evalbe, double *evalai, lng_st ist, par_st par)
{
  ...
  pf2 = fopen("detIntARLifetimesFinalStates.dat", "w");
  for (iDeltaE = 0; iDeltaE < ist.nConsWindows; iDeltaE++) {
    for (iBiexc = 0; iBiexc < ist.nBiexcitons; iBiexc++) {
      iBiexcDeltaE = iBiexc + iDeltaE*ist.nBiexcitons;
      // hole channel
      for (a = 0; a < ist.nElecs; a++) {
        for (i = 0; i < ist.itot; i++) {
          ...
          if (fabs(evalbe[a+ist.nHoles] - evalai[i] - biexcitonEnergies[iBiexc]) <= deltaE[iDeltaE]) { 
            ...
            fprintf(pf2, "hole channel: %ld %.6f %ld %.6f ", iDeltaE, deltaE[iDeltaE], iBiexc, biexcitonEnergies[iBiexc]);
            fprintf(pf2, "%ld %.8f ", a, evalbe[a+ist.nHoles]);
            fprintf(pf2, "%.8f\n", (AUTOPS*TWOPI/(2.0*deltaE[iDeltaE]))*w2h[iBiexc*ist.atot*ist.nElecs + a*ist.itot + i]);
            fflush(pf2);
          }
        }
      }
      ...
      for (a = 0; a < ist.atot; a++) {
        for (i = 0; i < ist.nHoles; i++) {
          ...
          if (fabs(evalai[a+ist.itot] - evalbe[i] - biexcitonEnergies[iBiexc]) <= deltaE[iDeltaE]) { 
            ...
            fprintf(pf2, "elec channel: %ld %.6f %ld %.6f ", iDeltaE, deltaE[iDeltaE], iBiexc, biexcitonEnergies[iBiexc]);
            fprintf(pf2, "%ld %.8f ", i, evalbe[i]);
            fprintf(pf2, "%.8f\n", (AUTOPS*TWOPI/(2.0*deltaE[iDeltaE]))*w2e[iBiexc*ist.atot*ist.nHoles + a*ist.nHoles + i]);
            fflush(pf2);
          }
        }
      }
                    ...
                    zomplex *potq, lng_st ist, par_st par, fftw_plan_loc *planfw, fftw_plan_loc *planbw, fftw_complex *fftwpsi) 
{
  ...
  for (iDeltaE = 0; iDeltaE < ist.nConsWindows; iDeltaE++) {
    fprintf(pf, "Energy conservation window = %.6f\n\n", deltaE[iDeltaE]);
    calcBoltzmannWeightedRates(biexcitonEnergies, &(eRate[iDeltaE*ist.nBiexcitons]), &(hRate[iDeltaE*ist.nBiexcitons]), par.temp, ist.nBiexcitons, pf, iDeltaE);
    writeSeparation(pf);
  }
  fclose(pf);
...
void calcDeterministicNonIntAR(double *vijck, double *vabck, double *evalbe, double *evalai, lng_st ist, par_st par) {
  ...
  pf2 = fopen("detNonIntARLifetimesFinalStates.dat", "w");
  for (iDeltaE = 0; iDeltaE < ist.nConsWindows; iDeltaE++) {
    for (iBiexc = 0; iBiexc < ist.nBiexcitons; iBiexc++) {
      iBiexcDeltaE = iBiexc + iDeltaE*ist.nBiexcitons;
      ...
      for (i = 0; i < nFinalHoles[iBiexcDeltaE]; i++) {
        ...
        fprintf(pf2, "hole channel: %ld %.6f %ld %.6f ", iDeltaE, deltaE[iDeltaE], iBiexc, biexcitonEnergies[iBiexc]);
        fprintf(pf2, "%ld %.8f ", iIndex, evalai[iIndex]);
        fprintf(pf2, "%.8f\n", (AUTOPS*TWOPI/(2.0*deltaE[iDeltaE]))*tmp*tmp);
        fflush(pf2);
      }
      for (a = 0; a < nFinalElecs[iBiexcDeltaE]; a++) {
        ...
        fprintf(pf2, "electron channel: %ld %.6f %ld %.6f ", iDeltaE, deltaE[iDeltaE], iBiexc, biexcitonEnergies[iBiexc]);
        fprintf(pf2, "%ld %.8f ", a, evalai[a+ist.itot]);
        fprintf(pf2, "%.8f\n", (AUTOPS*TWOPI/(2.0*deltaE[iDeltaE]))*tmp*tmp);
        fflush(pf2);
      }
      hRate[iBiexcDeltaE] *= (AUTOPS*TWOPI/(2.0*deltaE[iDeltaE]));
