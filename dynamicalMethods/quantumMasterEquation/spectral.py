...
"""
  ...
  Example run commands:
    $ python3.6 spectral.py --kT=0.01 --nTimeSteps=100 --lowestEnergyLevel=-0.2
    $ python3.6 spectral.py @input.par
...
def calcCurrent(D_level_Ltilde, D_level_L, d_level, num_operator_total, Ham, Rho, gammaLeft):
  ...
  I_current_max_1 = lind.Lind_1lead(D_level_Ltilde, D_level_L, d_level, Ham, Rho, gammaLeft)

  num_rho = np.einsum('ab,bc->ac', num_operator_total, Rho, optimize=True)

  I_current_max_2 = lind.Lind_1lead(D_level_Ltilde, D_level_L, d_level, Ham, num_rho, gammaLeft)
...
def main():
  ...
  # Diagonal terms of single-quasiparticle part of the total Hamiltonian 
  # Total Hamiltonian += sum_i,i_sigma e_i*d^dagger_i,i_sigma*d_i,i_sigma)
  for i in range(nLevels):
    for i_sigma in range(params.spinDegeneracy):
      Ham += spEnergyLevels[i]*np.einsum('ab,bc->ac', np.transpose(d_level[i, i_sigma]), d_level[i, i_sigma])

#  TODO: add in ability to read in off diagonal terms to the single-quasiparticle part of the total Hamiltonian
#  # Off-diagonal part of single-quasiparticle Hamiltonian (t_ij part)
#  for i in range(nLevels):
#    for i_sigma in range(params.spinDegeneracy):
#      for j in range(nLevels):
#        for j_sigma in range(params.spinDegeneracy):
#          Ham += spElecCoupling[i, j]*np.einsum('ab,bc->ac', np.transpose(d_level[i, i_sigma]), d_level[j, j_sigma])	  

  # Quasiparticle-Quasiparticle Coulomb interaction part of the total Hamiltonian
  # Includes the Hubbard U terms along with the exchange terms
  for i in range(nLevels):
    for i_sigma in range(params.spinDegeneracy):
  ...
  S2_operator_eigenbasis = np.real(np.einsum('ba,bc,cd->ad', evecs, S2_operator_total, evecs, optimize=True))
  Sz_operator_eigenbasis = np.real(np.einsum('ba,bc,cd->ad', evecs, Sz_operator_total, evecs, optimize=True))
  N_operator_eigenbasis = np.real(np.einsum('ba,bc,cd->ad', evecs, num_operator_total, evecs, optimize=True))
  ...
  for i_bias in range(bias.shape[0]): 
    ...
    for t in range(params.nTimeSteps): 
      ...
      Rho = lind.RK4(D_level_Ltilde, D_level_L, D_level_Rtilde, D_level_R, d_level, Ham, Rho, params.dt, gammaLeft, gammaRight)
